...
import { disable, useBackend, listBackends, diagnose, deletePassword, setPassword, getPassword, getCredential } from './index.js';
 ...
 */
async function showBackends() {
    const backends = await listBackends();
    for (const backend of backends) {
        console.log(`${backend.id}\t(priority: ${backend.priority})\t${backend.name}`);
    }
}
/**
 ...
 */
async function handleGet(service, username) {
    if (!service) {
    ...
    if (mode === "password") {
        ...
        if (password === null) {
            console.error(`Password not found for service '${service}' and user '${username}'.`);
            process.exitCode = 1;
            return;
        }
 ...
 */
async function handleSet(service, username) {
    if (!service || !username) {
 ...
 * @throws {Error} When service or username parameters are missing
 */
async function handleDelete(service, username) {
    if (!service || !username) {
...
function emitCredential(credential, format, includeUsername) {
    if (format === "json") {
